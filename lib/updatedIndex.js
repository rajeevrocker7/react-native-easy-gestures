...
import { PanResponder, View, Platform, Dimensions, Animated } from "react-native";
...
export default class Gestures extends Component {
  ...
  onMove = (event, gestureState) => {
    ...
    if (touches) {
      if (!initialTouches || (touches.length && touches.length !== initialTouches.length)) {
        ...
        // this.onPinch(event);
        //////For OutOfBounds
        this.onPinch(event, gestureState);
      }
    }
  ...
  onPinch = (event, gestureState) => {
    if (isMultiTouch(event)) {
      ...
      this.onScale(event, gestureState);
      this.onRotate(event, gestureState);
    }
  }
  ...
  updateNativeStyle = (style) => {
    this.view && this.view.setNativeProps({ style });
  }
  ...
  //CHANGES
  resetWithLastKnownStyle = (lastKnownStyle = { left: 0, top: 0, transform: [] }, callback) => {
    this.prevStyle = lastKnownStyle;
    const { left, top, transform } = this.prevStyle;
